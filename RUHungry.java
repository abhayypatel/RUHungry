...
public class RUHungry {
    ...
    private final Queue<Party> leftQueueVar;
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (int j = 0; j < count; j++) {
                ...
                int[] ids = new int[ingredientsCount];
                // Read ingredients
                for (int k = 0; k < ingredientsCount; k++) {
                    ids[k] = StdIn.readInt();
                }
                StdIn.readLine();
                Dish dish = new Dish(cats, name, ids);
                MenuNode node;
                if (menuVar[i] == null) {
                    node = new MenuNode(dish, null);
                } else {
                    node = new MenuNode(dish, menuVar[i]);
                }
                menuVar[i] = node;
            }
        }
    ...
    public void addStockNode(StockNode newNode) {
        ...
        int indexCompute = nodeID % stockVarSize;

        // Add a newNode
        if (stockVar[indexCompute] != null) {
            newNode.setNextStockNode(stockVar[indexCompute]);
        }
        stockVar[indexCompute] = newNode;
    }
    ...
    public StockNode findStockNode(int ingredientID) {
        int compute = ingredientID % stockVarSize;
        StockNode stockNode = stockVar[compute];
        while (stockNode != null) {
            if (stockNode.getIngredient().getID() == ingredientID) {
                return stockNode;
            }
            stockNode = stockNode.getNextStockNode();
        }
        return null;
    }
    ...
    /**
     * PICK UP LINE OF THE METHOD:
     * *are you a single âforâ loop? cuz i only have iâs for you*
     * <p>
     * ***********
    ...
    public void updatePriceAndProfit() {
        for (MenuNode node : menuVar) {
            MenuNode menu = node;
            while (menu != null) {
                Dish dishNode = menu.getDish();
                int[] ids = dishNode.getStockID();
                double cost = 0;
                for (int id : ids) {
                    cost += findStockNode(id).getIngredient().getCost();
                    double priceCompute = cost * 1.2;
                    dishNode.setPriceOfDish(priceCompute);
                    dishNode.setProfit(priceCompute - cost);
                }
    ...
    public void createStockHashTable(String inputFile) {
        ...
        while (StdIn.hasNextLine()) {
            ...
            StdIn.readChar();
            String foodName = StdIn.readLine();
            double foodCost = StdIn.readDouble();
            StdIn.readChar();
            int amount = StdIn.readInt();
            StdIn.readLine();

            Ingredient item = new Ingredient(readID, foodName, amount, foodCost);
            StockNode node = new StockNode(item, null);
            addStockNode(node);
        }
    }
    ...
    public boolean checkDishAvailability(String dishName, int numberOfDishes) {
        ...
        int[] ids = menuNode.getDish().getStockID();
        for (int id : ids) {
            StockNode stockNode = findStockNode(id);
            int amount = stockNode.getIngredient().getStockLevel();
            if (numberOfDishes > amount) {
                available = false;
                break;
            }
        }
     ...
     * together forever because Iâll always stay true to you*
     * <p>
     * ***************
    ...
    public void order(String dishName, int quantity) {
        MenuNode menuFirst = findDish(dishName); // the ordered dish by user
        boolean isTrue = checkDishAvailability(dishName, quantity);
        if (isTrue) {
            Dish dishNode = menuFirst.getDish();
            double profit = dishNode.getProfit();
            double totalProfit = profit * quantity;
            TransactionData data = new TransactionData("order", dishName, quantity,
                    totalProfit, true);
            ...
            TransactionData transNode = new TransactionData("order", dishName, quantity,
                    0, false);
            addTransactionNode(transNode);
            MenuNode menuSecond = menuFirst.getNextMenuNode();
            while (menuSecond != null) {
                String reorderName = menuSecond.getDish().getName();
                if (!checkDishAvailability(reorderName, quantity)) {
                    TransactionData reorder = new TransactionData("order",
                            reorderName, quantity, 0, false);
                    addTransactionNode(reorder);
                } else {
                    break;
                }
                menuSecond = menuSecond.getNextMenuNode();
            }
            if (menuSecond == null) {
                String cats = menuFirst.getDish().getCategory();
                int index = findCategoryIndex(cats);
                menuSecond = menuVar[index];
                while (menuSecond != menuFirst) {
                    String reorderName = menuSecond.getDish().getName();
                    if (!checkDishAvailability(reorderName, quantity)) {
                        TransactionData failedReorder = new TransactionData("order",
                                reorderName, quantity, 0, false);
                        addTransactionNode(failedReorder);
                    } else {
                        break;
                    }
                    menuSecond = menuSecond.getNextMenuNode();
            ...
            if (menuSecond != menuFirst) {
                Dish dishNode = menuSecond.getDish();
                double profit = dishNode.getProfit();
                double totalProfit = profit * quantity;
                TransactionData data = new TransactionData("order", dishNode.getName(), quantity,
                        totalProfit, true);
                addTransactionNode(data);
                int[] idArr = dishNode.getStockID();
                for (int id : idArr) {
                    updateStock(null, id, -quantity);
                }
            }
    ...
    public double profit() {
        double profit = 0;
        TransactionNode transNode = transactionVar;
        while (transNode != null) {
            profit += transNode.getData().getProfit();
    ...
    public void donation(String ingredientName, int quantity) {
        int amount = findStockNode(ingredientName).getIngredient().getStockLevel();
        TransactionData node;
        if (profit() > 50.0 && (amount >= quantity)) {
            updateStock(ingredientName, -1, -quantity);
            node = new TransactionData("donation", ingredientName, quantity,
                    0.0, true);
        } else {
            node = new TransactionData("donation", ingredientName, quantity,
                    0.0, false);
    ...
    public void restock(String ingredientName, int quantity) {
        ...
        double costFinal = cost * quantity;
        TransactionData node;
        if (profit() >= costFinal) {
            updateStock(ingredientName, -1, quantity);
            node = new TransactionData("restock", ingredientName, quantity,
                    -costFinal, true);
        } else {
            node = new TransactionData("restock", ingredientName, quantity,
                    0.0, false);
    ...
    public void printRestaurant() {
            ...
            for (int i = 0; i < categoryVar.length; i++) {
                ...
                while (ptr != null) {
                    StdOut.print(ptr.getDish().getName() + "  Price: $"
                            ...
                            + " Profit: $" + ((Math.round(ptr.getDish().getProfit() * 100.0))
                                    / 100.0));
                    StdOut.println();
        ...
        if (stockVar != null) {
            for (int i = 0; i < 10; i++) {
                ...
                while (ptr != null) {
                    StdOut.print(
                            ptr.getIngredient().getName() + "  ID: " + ptr.getIngredient().getID()
                                    + " Price: " + ((Math.round(ptr.getIngredient().getCost() * 100.0)) / 100.0)
        ...
        if (transactionVar != null) {
            ...
            while (ptr != null) {
                ...
                StdOut.println(
                        "Type: " + type + ", Name: " + item + ", Amount: " + amount + ", Profit: $"
                                + ((Math.round(profit * 100.0)) / 100.0) + ", Was it a Success? "
                                + success);
        ...
        Queue<Party> leftQueue = leftQueueVar;
